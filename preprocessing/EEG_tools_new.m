...
classdef EEG_tools_new
    ...
    methods (Static)
        %% get_file_path
        function [fileDir,fileList] = getFilePath(raw,op,save_path)
...
%                 data.Data = detrend(input);
        ...
        %% bandpass filter
        function data = bandpass(data, input,freq_band_Ecog,freq_band_EMG, sample_rate)
            bandpass_default_Ecog = designfilt('bandpassiir', 'FilterOrder',4, ...
                'PassbandFrequency1', freq_band_Ecog(1), ...
                'PassbandFrequency2', freq_band_Ecog(2), ...
                'SampleRate', sample_rate);

            bandpass_default_EMG = designfilt('bandpassiir', 'FilterOrder',4, ...
                'PassbandFrequency1', freq_band_EMG(1), ...
                'PassbandFrequency2', freq_band_EMG(2), ...
                'SampleRate', sample_rate);

            if data.replace == true
                register = [];
                for i=1:1:size(input, 1)/2
                    register = [register ; filtfilt(bandpass_default_Ecog, input(i, :))];
                end

                for i = size(input,1)/2+1 :1:size(input,1)
                    register = [register ; filtfilt(bandpass_default_EMG, input(i, :))];
                end
...
%                 data.Timestamps = downsample(data.Timestamps, skip);
        ...
        %% preprocessing
        function data = Preprocessing(data, input, freq_band_Ecog,freq_band_EMG, sample_rate, fs_down)
            data = EEG_tools_new.detrend(data, input);
            data = EEG_tools_new.reduce_mean(data, input);
            data = EEG_tools_new.notch60(data, input);
            data = EEG_tools_new.bandpass(data, input,freq_band_Ecog,freq_band_EMG, sample_rate);
            data = EEG_tools_new.down_sample(data, input, fs_down);
